  ...
  <div class="app-container">
    ...
    <div class="page-hero">
      <div class="hero-inner">
        <div class="hero-text">
          <h1 class="hero-title">Levene 检验</h1>
          <p class="hero-desc">
            <strong>适用场景：</strong>多组临床试验数据方差齐性评估（数据不满足正态分布时）、多中心研究各中心变异度一致性检验、
            ANOVA 前置检验（尤其数据偏态或含离群值时）。基于各观测值与组均数的绝对偏差做单因素方差分析，
            检验 H₀: σ₁² = σ₂² = … = σₖ²，比 Bartlett 检验对非正态数据更稳健
          </p>
        </div>
        <el-tag class="hero-tag" effect="dark" round>VARIANCE · LEVENE</el-tag>
      </div>
    </div>
        ...
        <el-card shadow="never" class="input-card">
          ...
          <div v-if="inputMode === 'table'" class="spread-area">
            ...
            <div class="spread-grid">
              ...
              <div class="spread-body">
                <div v-for="i in rowCount" :key="i" class="spread-row" :class="{ 'sp-even': i % 2 === 0 }">
                  ...
                  <div class="sp-data-cell"><input v-model="tableC2[i-1]" class="sp-input" type="text" inputmode="decimal" @keydown.tab.prevent="focusCell(i-1, 2)" @keydown.enter.prevent="focusCell(i, 0)" :ref="el => setCellRef(el, i-1, 1)" /></div>
                  <div class="sp-data-cell"><input v-model="tableC3[i-1]" class="sp-input" type="text" inputmode="decimal" @keydown.tab.prevent="focusCell(i, 0)" @keydown.enter.prevent="focusCell(i, 0)" :ref="el => setCellRef(el, i-1, 2)" /></div>
            ...
            <div class="spread-legend">
              ...
              <span class="legend-hint">C4~C{{ totalCols }} 暂未使用</span>
            </div>
          </div>

          <div v-else>
            <div class="input-tip"><el-icon class="tip-icon"><Warning /></el-icon><span>每行一组数据，组内用逗号分隔。至少输入 2 组。</span></div>
            <el-form label-position="top"><el-form-item label="各组数据（每行一组）"><el-input v-model="form.rawData" type="textarea" :rows="5" placeholder="72,68,75,80,65,90&#10;55,58,62,60,57,63&#10;48,52,45,50,47,53" /></el-form-item></el-form>
          </div>
      ...
      <el-col :lg="8" :xs="24">
        <div class="param-sidebar">
          ...
          <div class="principle-content">
            <div class="principle-block"><div class="principle-label">假设检验</div><p>H₀: σ₁² = σ₂² = … = σₖ²</p><p>H₁: 至少两组方差不等</p></div>
            <div class="principle-block"><div class="principle-label">检验步骤</div><p>1. 计算 zᵢⱼ = |xᵢⱼ − x̄ᵢ|（绝对偏差）</p><p>2. 对 zᵢⱼ 做单因素 ANOVA</p><p>3. 求 F 统计量及 P 值</p></div>
            <div class="principle-block"><div class="principle-label">检验统计量</div><div class="formula-box">F = MSB(z) / MSW(z)</div></div>
            <div class="principle-block"><div class="principle-label">适用条件</div><p>不要求正态性；对偏态数据和含离群值的数据稳健</p></div>
            <div class="principle-block"><div class="principle-label">与其他方法对比</div><p>Levene：稳健，适用范围广</p><p>Bartlett：正态假定下更灵敏</p><p>Brown-Forsythe：用中位数替代均数，更稳健</p></div>
          </div>
          <div class="ref-section">
            <div class="ref-title">参考文献</div>
            <p class="ref-item">[1] 方积乾.《卫生统计学》第7版, 人民卫生出版社, 2012.</p>
            <p class="ref-item">[2] Levene H. Robust tests for equality of variances. In: Olkin I (ed), Contributions to Probability and Statistics, Stanford Univ Press, 1960: 278-292.</p>
            <p class="ref-item">[3] Brown MB, Forsythe AB. Robust tests for the equality of variances. JASA, 1974, 69(346): 364-367.</p>
          </div>
        </div>
      </el-col>
    </el-row>
        ...
        <el-row :gutter="20" class="mb-4 equal-row">
          <el-col :lg="12" :xs="24" class="mb-4">
            <el-card shadow="never" class="detail-card">
              <template #header><div class="card-header-inner"><el-icon class="header-icon"><TrendCharts /></el-icon><span class="font-bold">各组绝对偏差均值对比</span></div></template>
          ...
          <el-col :lg="12" :xs="24" class="mb-4">
            <el-card shadow="never" class="detail-card">
              ...
              <el-table :data="groupTable" size="small" stripe border>
                ...
                <el-table-column prop="meanZ" label="平均|偏差|" min-width="90" />
              </el-table>
            </el-card>
          </el-col>
          <el-col :lg="12" :xs="24" class="mb-4">
...
const dataCount = computed(() => {
  let c = 0;
  if (tableC1.value.filter(v => v.trim()).length >= 2) c++;
  if (tableC2.value.filter(v => v.trim()).length >= 2) c++;
  if (tableC3.value.filter(v => v.trim()).length >= 2) c++;
  return c;
});
...
function addRows(n: number) { for (let i = 0; i < n; i++) { tableC1.value.push(""); tableC2.value.push(""); tableC3.value.push(""); } }
async function pasteData() {
  try { const text = await navigator.clipboard.readText(); const lines = text.trim().split("\n"); lines.forEach((line, i) => { const p = line.split(/[,\t]+/); if (i < tableC1.value.length) { if (p[0]) tableC1.value[i] = p[0].trim(); if (p[1]) tableC2.value[i] = p[1].trim(); if (p[2]) tableC3.value[i] = p[2].trim(); } }); } catch { ElMessage.info("请允许剪贴板访问"); }
}
...
function loadDemo() {
  if (inputMode.value === "table") {
    const d1 = [72, 68, 75, 80, 65, 90, 78, 82]; const d2 = [55, 58, 62, 60, 57, 63, 59, 61]; const d3 = [48, 52, 45, 50, 47, 53, 46, 51];
    tableC1.value = Array(20).fill(""); tableC2.value = Array(20).fill(""); tableC3.value = Array(20).fill("");
    d1.forEach((v, i) => tableC1.value[i] = String(v)); d2.forEach((v, i) => tableC2.value[i] = String(v)); d3.forEach((v, i) => tableC3.value[i] = String(v));
  } else { form.rawData = "72,68,75,80,65,90,78,82\n55,58,62,60,57,63,59,61\n48,52,45,50,47,53,46,51"; }
  calculate();
}

function clearAll() { tableC1.value = Array(20).fill(""); tableC2.value = Array(20).fill(""); tableC3.value = Array(20).fill(""); form.rawData = ""; res.value = false; }
...
function calculate() {
  ...
  const allZ: number[] = []; zGroups.forEach(zg => zg.forEach(z => allZ.push(z)));
  ...
  const meanZs = zGroups.map(zg => S.mean(zg));

  metrics.value = [
    { label: "组数 (k)", value: k, type: "accent" },
    { label: "总 N", value: N, type: "accent" },
    { label: "F(Levene)", value: S.fmt(F), type: "warning" },
    { label: "df₁, df₂", value: `${df1}, ${df2}`, type: "success" },
  ...
  groupTable.value = groups.map((g, i) => ({
    group: `组${i + 1}`, n: g.length, mean: S.fmt(S.mean(g)), sd: S.fmt(S.stdDev(g)), variance: S.fmt(vs[i]), meanZ: S.fmt(meanZs[i]),
  }));
  ...
  barChartOpts.value = {
    ...
    yAxis: { type: "value", name: "平均绝对偏差" },
    series: [{ type: "bar", data: meanZs.map(v => +v.toFixed(4)), itemStyle: { color: "#4558d0", borderRadius: [4, 4, 0, 0] }, barWidth: "40%" }],
  };
  ...
  const varTexts = vs.map((v, i) => `组${i + 1} = ${S.fmt(v)}`).join("，");
  narrative.value = `<p>共 <strong>${k}</strong> 组独立样本，总样本量 N = <strong>${N}</strong>。各组样本量：${ns.join("、")}。</p>` +
    `<p>各组方差分别为：${varTexts}。</p>` +
    `<p>Levene 检验先计算各观测值与组均数的绝对偏差 zᵢⱼ = |xᵢⱼ − x̄ᵢ|，再对 z 值做单因素 ANOVA。各组平均绝对偏差：${meanZs.map((v, i) => `组${i+1} = ${S.fmt(v)}`).join("，")}。</p>` +
    `<p>F(Levene) = <strong>${S.fmt(F)}</strong>，df = (${df1}, ${df2})，P = <strong>${S.fmtP(pVal)}</strong>。${sig
      ? `P < 0.05，<strong>拒绝 H₀</strong>，各组总体方差不全相等。建议使用 Welch 校正或非参数检验。`
      : `P ≥ 0.05，<strong>不拒绝 H₀</strong>，方差齐性假设成立，可进行方差分析。`}</p>` +
    `<p><em>Levene 检验不要求正态性，适用面广。如数据正态，Bartlett 检验检验力更高。</em></p>`;
}
</script>
...
.page-hero::before { content: "L"; position: absolute; right: 40px; top: 50%; transform: translateY(-50%); font-size: 88px; font-weight: 900; opacity: 0.06; color: #4558d0; font-family: "Georgia", serif; pointer-events: none; }
...
.c1-dot { background: #4558d0; } .c2-dot { background: #22c55e; } .c3-dot { background: #e6a23c; }
