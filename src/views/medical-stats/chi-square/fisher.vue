        ...
        <el-card shadow="never" class="input-card">
          <div class="ff-table-area">
            <table class="fourfold-table">
              <thead>
                <tr><th class="ft-corner" /><th>阳性</th><th>阴性</th><th class="ft-dim">合计</th></tr>
              </thead>
              <tbody>
                <tr>
      ...
      <el-col :lg="8" :xs="24">
        <div class="param-sidebar">
          ...
          <div class="principle-content">
            ...
            <div class="principle-block">
              <div class="principle-label">检验方法</div>
              <p>基于超几何分布，精确计算在边际固定条件下观测到当前或更极端结果的概率。</p>
            </div>
            <div class="principle-block">
              ...
              <div class="condition-list">
                <div class="condition-item ok">
                  <span class="cond-dot" />
                  <span>总样本量 N 较小（N &lt; 40）</span>
                </div>
                <div class="condition-item warn">
                  <span class="cond-dot" />
                  <span>理论频数 T &lt; 5 的格子 &gt; 20%</span>
                </div>
                <div class="condition-item danger">
                  <span class="cond-dot" />
                  <span>理论频数 T &lt; 1 的格子存在</span>
                </div>
              </div>
            </div>
          </div>
...
function calculate() {
  ...
  const sig = fisher.pTwoSided < 0.05;

  result.value = true;

  resultMetrics.value = [
    { label: "精确概率", value: S.fmt(fisher.pExact, 6), type: "accent" },
    { label: "双侧 P 值", value: S.fmt(fisher.pTwoSided, 6), type: Number(fisher.pTwoSided) < 0.05 ? "warning" : "neutral" },
    { label: "χ² P 值 (对比)", value: S.fmt(pChi2, 6), type: pChi2 < 0.05 ? "warning" : "neutral" },
    { label: "结论 (α=0.05)", value: sig ? "差异有统计学意义" : "差异无统计学意义", type: sig ? "warning" : "neutral" },
  ];

  compareRows.value = [
    { method: "Fisher 精确检验", stat: S.fmt(fisher.pExact, 6), pVal: S.fmt(fisher.pTwoSided, 6) },
    { method: "Pearson χ²", stat: S.fmt(chi2, 4), pVal: S.fmt(pChi2, 6) },
  ];

  const p1 = n1 > 0 ? a / n1 : 0, p2 = n2 > 0 ? c / n2 : 0;
  narrativeHtml.value = `
    <p>本次 Fisher 精确检验共纳入 <strong>${n}</strong> 例样本。</p>
    <p>组一阳性率 = <strong>${S.fmt(p1, 4)}</strong>（${a}/${n1}），组二阳性率 = <strong>${S.fmt(p2, 4)}</strong>（${c}/${n2}）。</p>
    <p>Fisher 精确概率 = <strong>${S.fmt(fisher.pExact, 6)}</strong>，双侧 P = <strong>${S.fmt(fisher.pTwoSided, 6)}</strong>。</p>
    <p>Pearson χ² = ${S.fmt(chi2, 4)}，P = ${S.fmt(pChi2, 6)}（仅供参考，小样本下可能不准确）。</p>
    <p>在 α = 0.05 水平下，${sig
      ? `P < 0.05，<strong>拒绝 H₀</strong>，认为两组阳性率差异有统计学意义。`
      : `P ≥ 0.05，<strong>不拒绝 H₀</strong>，尚不能认为两组阳性率差异有统计学意义。`
    }</p>
