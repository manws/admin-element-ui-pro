<template>
  <div class="app-container">
    <div class="page-hero"><div class="hero-inner"><div class="hero-text">
      ...
      <p class="hero-desc"><strong>适用场景：</strong>总体标准差 σ 已知时的均数假设检验（如已知历史参考值的质控样品检验、大样本率的假设检验）、标准化检测指标与参考值对比。通过 Z 统计量检验样本均数与已知总体均数 μ₀ 之间是否存在统计学差异（H₀: μ = μ₀），支持双侧/单侧检验，同时计算 95% 置信区间</p>
...
function calculate() {
  ...
  narrativeHtml.value = `<p>样本量 n = <strong>${n}</strong>，样本均数 x̄ = <strong>${S.fmt(xbar)}</strong>。总体参数：μ₀ = ${mu0}，σ = ${sigma}。</p>` +
    `<p>标准误 SE = σ/√n = ${sigma}/√${n} = <strong>${S.fmt(se)}</strong>。</p>` +
