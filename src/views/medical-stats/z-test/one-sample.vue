<template>
  <div class="app-container">
    <div class="page-hero"><div class="hero-inner"><div class="hero-text">
      <h1 class="hero-title">单样本 Z 检验</h1>
      <p class="hero-desc">适用于总体标准差 σ 已知的情况，通过 Z 统计量检验样本均数与已知总体均数 μ₀ 之间是否存在统计学差异，支持双侧/左侧/右侧检验，同时计算 95% 置信区间</p>
    </div><el-tag class="hero-tag" effect="dark" round>Z-TEST · ONE-SAMPLE</el-tag></div></div>

    <el-row :gutter="20" class="mb-4 input-row">
      <el-col :lg="16" :xs="24">
        <el-card shadow="never" class="input-card">
          <div class="ff-table-area"><table class="fourfold-table">
            <thead><tr><th>总体均数 (μ₀)</th><th>总体标准差 (σ)</th><th>样本量 (n)</th><th>样本均数 (x̄)</th></tr></thead>
            <tbody><tr>
              <td class="ft-input"><el-input-number v-model="form.mu0" :step="1" :controls="false" class="fourfold-input" /></td>
              <td class="ft-input"><el-input-number v-model="form.sigma" :min="0.001" :step="1" :controls="false" class="fourfold-input" /></td>
              <td class="ft-input"><el-input-number v-model="form.n" :min="1" :step="1" :controls="false" class="fourfold-input" /></td>
      ...
      <el-col :lg="8" :xs="24">
        <div class="param-sidebar"><div class="param-sidebar-header"><el-icon class="sidebar-icon"><InfoFilled /></el-icon>检验原理</div>
          <div class="principle-content">
            <div class="principle-block"><div class="principle-label">假设检验</div><p>H₀: μ = μ₀，H₁: μ ≠ μ₀（双侧）</p></div>
            <div class="principle-block"><div class="principle-label">检验统计量</div><div class="formula-box">Z = (x̄ − μ₀) / (σ / √n)</div></div>
            <div class="principle-block"><div class="principle-label">适用条件</div><p>总体标准差 σ 已知，或大样本（n ≥ 30）时可用样本标准差替代</p></div>
          </div>
...
function calculate() {
  ...
  const pVal = tail === "two" ? pTwo : tail === "right" ? (z > 0 ? pTwo / 2 : 1 - pTwo / 2) : (z < 0 ? pTwo / 2 : 1 - pTwo / 2);
  const zC = S.normInv(0.975), ciL = xbar - zC * se, ciU = xbar + zC * se, sig = pVal < 0.05;
  res.value = true;
  metrics.value = [
    { label: "Z 统计量", value: S.fmt(z), type: "accent" }, { label: "标准误 (SE)", value: S.fmt(se), type: "success" },
    ...
    { label: "结论 (α=0.05)", value: sig ? "拒绝H₀" : "不拒绝H₀", type: sig ? "warning" : "neutral" },
  ];
  const tailText = tail === "two" ? "双侧" : "单侧";
  narrativeHtml.value = `<p>样本量 n = ${n}，x̄ = <strong>${S.fmt(xbar)}</strong>，μ₀ = ${mu0}，σ = ${sigma}，SE = ${S.fmt(se)}。</p><p>Z = <strong>${S.fmt(z)}</strong>，P(${tailText}) = <strong>${S.fmtP(pVal)}</strong>。${sig ? `P < 0.05，<strong>拒绝 H₀</strong>，差异有统计学意义。` : `P ≥ 0.05，<strong>不拒绝 H₀</strong>。`}</p><p>95% CI: [${S.fmt(ciL)}, ${S.fmt(ciU)}]。</p>`;
}
onMounted(calculate);
</script>
<style scoped>
...
.ff-table-area { display: flex; justify-content: center; padding: 16px 0; } .fourfold-table { width: 100%; max-width: 700px; border-collapse: collapse; font-size: 14px; } .fourfold-table thead { border-top: 2px solid var(--el-text-color-primary); border-bottom: 1px solid var(--el-text-color-primary); } .fourfold-table th, .fourfold-table td { padding: 14px 16px; text-align: center; } .fourfold-table th { font-weight: 600; font-size: 13px; } .fourfold-table tbody tr { border-bottom: 2px solid var(--el-text-color-primary); } .ft-input { padding: 10px 12px; } .fourfold-input { width: 100%; max-width: 130px; }
