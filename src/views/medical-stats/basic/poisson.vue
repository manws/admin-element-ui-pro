      ...
      <el-col :lg="16" :xs="24">
        <el-card shadow="never" class="input-card">
          <el-tabs v-model="activeTab">
            <el-tab-pane name="prob">
              ...
              <div class="ff-table-area">
                <table class="fourfold-table">
                  <thead><tr><th>期望值 (μ)</th><th>观察值 (X)</th></tr></thead>
                  <tbody><tr>
                    <td class="ft-input"><el-input-number v-model="prob.mu" :min="0.1" :max="100" :step="0.5" :precision="1" :controls="false" class="fourfold-input" /></td>
                    <td class="ft-input"><el-input-number v-model="prob.x" :min="0" :max="200" :step="1" :controls="false" class="fourfold-input" /></td>
                  </tr></tbody>
                </table>
              </div>
            ...
            <el-tab-pane name="ci">
              <template #label><span class="tab-label"><el-icon><TrendCharts /></el-icon>率的区间估计</span></template>
              <div class="ff-table-area">
                <table class="fourfold-table">
                  <thead><tr><th>事件数 (X)</th><th>观察单位 (T)</th><th>置信水平</th></tr></thead>
                  <tbody><tr>
                    <td class="ft-input"><el-input-number v-model="ci.x" :min="0" :max="10000" :step="1" :controls="false" class="fourfold-input" /></td>
                    <td class="ft-input"><el-input-number v-model="ci.t" :min="1" :max="100000" :step="100" :controls="false" class="fourfold-input" /></td>
                    <td class="ft-input"><el-select v-model="ci.alpha" style="width:120px"><el-option :value="0.05" label="95%" /><el-option :value="0.01" label="99%" /></el-select></td>
                  </tr></tbody>
                </table>
              </div>
            ...
            <el-tab-pane name="compare">
              ...
              <div class="ff-table-area">
                <table class="fourfold-table">
                  <thead><tr><th class="ft-corner" /><th>事件数</th><th>观察单位</th></tr></thead>
                  <tbody>
                    <tr>
                      <td class="ft-label"><span class="group-badge g1">组 一</span></td>
                      <td class="ft-input"><el-input-number v-model="cmp.x1" :min="0" :step="1" :controls="false" class="fourfold-input" /></td>
                      <td class="ft-input"><el-input-number v-model="cmp.t1" :min="1" :step="100" :controls="false" class="fourfold-input" /></td>
                    </tr>
                    <tr>
                      <td class="ft-label"><span class="group-badge g2">组 二</span></td>
                      <td class="ft-input"><el-input-number v-model="cmp.x2" :min="0" :step="1" :controls="false" class="fourfold-input" /></td>
                      <td class="ft-input"><el-input-number v-model="cmp.t2" :min="1" :step="100" :controls="false" class="fourfold-input" /></td>
                    </tr>
                  </tbody>
                </table>
              </div>
              <div class="action-bar"><el-button type="primary" @click="calcCmp" class="calc-btn"><el-icon class="mr-1"><DataAnalysis /></el-icon>计算</el-button></div>
            </el-tab-pane>
          </el-tabs>
        </el-card>
      </el-col>
      ...
      <el-col :lg="8" :xs="24">
        <div class="param-sidebar">
          ...
          <div class="principle-content">
            <div class="principle-block"><div class="principle-label">Poisson 分布</div><p>X ~ P(μ)，描述单位时间/空间内稀有事件发生次数的分布。</p></div>
            <div class="principle-block"><div class="principle-label">概率公式</div><div class="formula-box">P(X=k) = e^(-μ) · μ^k / k!</div></div>
            <div class="principle-block"><div class="principle-label">性质</div><p>E(X) = Var(X) = μ</p></div>
          </div>
        </div>
      </el-col>
    </el-row>
...
import { Histogram, TrendCharts, Grid, InfoFilled, DataAnalysis, ChatLineSquare } from "@element-plus/icons-vue";
...
function calcProb() {
  ...
  probChartOpts.value = {
    ...
    series: [{ type: "bar", data: xs.map((k) => ({ value: +S.poissonPMF(k, mu).toFixed(6), itemStyle: { color: k === x ? "#E6A23C" : "#409EFF", borderRadius: [3, 3, 0, 0] } })) }],
  };
...
function calcCI() {
  const rate = ci.x / ci.t, se = Math.sqrt(ci.x) / ci.t;
  const z = S.normInv(1 - ci.alpha / 2);
  const lower = Math.max(0, rate - z * se), upper = rate + z * se;
...
.page-hero::before { content: "λ"; position: absolute; right: 40px; top: 50%; transform: translateY(-50%); font-size: 120px; font-weight: 900; opacity: 0.04; color: var(--el-color-primary); font-family: "Georgia", serif; pointer-events: none; }
